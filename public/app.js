(() => {
  ...
  if (tg) {
    tg.ready();
    tg.expand();
    try { tg.setHeaderColor('#1f3864'); } catch (e) {}
    try { tg.setBackgroundColor('#eceff3'); } catch (e) {}
  }
  ...
  // -------------------------------------------------------------------
  // WEBSOCKET (avtomatik qayta ulanish bilan)
  // -------------------------------------------------------------------
  let ws;
  let wsReady = false;
  ...
  function connect() {
    ...
    ws.onopen = () => {
      ...
      // Agar ulanish uzilishidan oldin qidiruv/suhbatda bo'lsak, holatni tiklaymiz
      if (wasReconnect) {
        if (currentChatScreen === 'chat') {
          addSystemMessage('Ulanish tiklandi. Xavfsizlik uchun yangi suhbatdosh qidiramiz.');
          wsSend({ type: 'find_partner' });
          setChatFlowScreen('search');
        } else if (currentChatScreen === 'search') {
          wsSend({ type: 'find_partner' });
        }
      }
    };
    ...
    ws.onclose = () => {
      wsReady = false;
      if (authFailed) return; // qayta urinishning foydasi yo'q - sabab hal qilinmaguncha
      showConnBanner();
      scheduleReconnect();
    };
  ...
  function handleServerEvent(data) {
    switch (data.type) {
      case 'error':
        if (data.fatal) {
          authFailed = true;
          const msg = data.message || 'Ulanishda xatolik yuz berdi.';
          if (tg && tg.showAlert) {
            tg.showAlert(msg);
          } else {
            alert(msg);
          ...
          addSystemMessage(`Xato: ${data.message}`);
        }
        break;
      ...
      case 'matched':
        ...
        addSystemMessage(`${escapeHtml(data.partner.name)} bilan ulandingiz.`);
        break;
      ...
      case 'partner_left':
        addSystemMessage("Suhbatdosh chiqib ketdi. Yangisini topish uchun \"Keyingisi\" tugmasini bosing.");
        break;
      ...
      case 'report_sent':
        addSystemMessage("Shikoyatingiz adminga yuborildi. Javob shu yerga kelmaydi, admin sizga botdan alohida yozadi.");
        break;
  ...
  function renderFriends(friends) {
    ...
    if (!friends || !friends.length) {
      list.innerHTML =
        '<p class="empty-hint">Hali do\'stlaringiz yo\'q.<br>Suhbatda yurak belgisini bosib do\'st qo\'shing.</p>';
      return;
    }
    list.innerHTML = '';
    friends.forEach((f) => {
