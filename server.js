...
// ---------------------------------------------------------------------------
// CRASH HIMOYASI — kutilmagan xato butun serverni "o'ldirib" qo'ymasligi uchun.
// Bunsiz bitta kutilmagan xato (masalan noto'g'ri xabar formati) butun
// serverni yiqitib, Render uni qayta ishga tushirishga majbur bo'ladi —
// bu tashqaridan xuddi "uxlab qolgandek" ko'rinadi va barcha faol
// suhbatlar uziladi.
// ---------------------------------------------------------------------------
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    chatId,
    `Assalomu alaykum, ${name}.\n\nBu yerda tasodifiy odamlar bilan birma-bir suhbatlashishingiz mumkin. Yoqqan suhbatdoshni "Yoqtirish" tugmasi bilan do'stlar ro'yxatiga qo'shib qo'yasiz (buning uchun ikkalangiz ham bosishingiz kerak).\n\nBoshlash uchun pastdagi tugmani bosing.`,
    options
  );
});
...
wss.on('connection', (ws, req) => {
  ...
  try {
    ...
    if (!tgUser) {
      console.log('❌ WS auth muvaffaqiyatsiz — initData tekshiruvidan o\'tmadi.');
      send(ws, {
        type: 'error',
        fatal: true,
        message: 'Telegram orqali tasdiqlanmadi. Ilovani Telegram bot ichidan oching.',
      });
      ws.close();
      return;
    }
    ...
    console.log(`🟢 Ulandi: ${client.name} (id: ${client.id})`);

    // Heartbeat uchun: bu socket "tirikligini" belgilaymiz
    ws.isAlive = true;
    ws.on('pong', () => {
      ws.isAlive = true;
    });
    ...
    switch (data.type) {
      case 'find_partner': {
        ...
        if (result) {
          ...
          console.log(`🔗 Juftlashdi: ${client.name}(${client.id}) <-> ${partner.name}(${partner.id})`);
          send(partner.ws, {
            type: 'matched',
          ...
          console.log(`⏳ Navbatda kutmoqda: ${client.name}(${client.id})`);
      ...
      case 'like_partner': {
        const partner = matchmaker.getRoomPartner(client.id);
        if (!partner) break;
        const { newMatch } = db.likeUser(client.id, partner.id);
        send(ws, { type: 'like_ack' });
        if (newMatch) {
          console.log(`💚 Yangi do'stlik: ${client.name}(${client.id}) <-> ${partner.name}(${partner.id})`);
  ...
  ws.on('close', () => {
    if (!client) return;
    console.log(`🔴 Uzildi: ${client.name} (id: ${client.id})`);
    const partner = matchmaker.getRoomPartner(client.id);
    matchmaker.leaveRoom(client.id);
    matchmaker.removeFromQueue(client.id);
    if (partner) send(partner.ws, { type: 'partner_left' });
...
// Har 30 soniyada barcha ulanishlarni "ping" qilamiz. Agar avvalgi ping'ga
// javob (pong) kelmagan bo'lsa — ulanish o'lik, uni yopamiz. Bu "yopish"
// yuqoridagi ws.on('close', ...) ni ishga tushiradi va matchmaker/navbat
// avtomatik tozalanadi (masalan telefon tarmog'i uzilib qolgan foydalanuvchi
// suhbatda "abadiy osilib" qolmasligi uchun).
...
// ---------------------------------------------------------------------------
// SELF-PING — Render bepul tarifida 15 daqiqa harakatsizlikdan keyin server
// "uxlab qoladi" va keyingi so'rovda sekin uyg'onadi (cold start). Buning
// oldini olish uchun ilova har 10 daqiqada o'zining /health manziliga
// so'rov yuborib, o'zini uyg'oq ushlab turadi.
// ---------------------------------------------------------------------------
if (WEBAPP_URL) {
  const PING_INTERVAL_MS = 10 * 60 * 1000; // 10 daqiqa

  setInterval(() => {
    fetch(`${WEBAPP_URL}/health`)
      .then(() => console.log(`🏓 Self-ping OK (${new Date().toLocaleTimeString('uz-UZ')})`))
      .catch((err) => console.log('🏓 Self-ping xato:', err.message));
  }, PING_INTERVAL_MS);

  console.log("🏓 Self-ping yoqildi — server har 10 daqiqada o'zini uyg'otib turadi.");
} else {
  console.log('⚠️  WEBAPP_URL berilmagani uchun self-ping ishlamaydi (Environment ga qo\'shing).');
}
